...
#ifndef CSPSTRUCTS_HEADER_FILE
#define CSPSTRUCTS_HEADER_FILE
...
#define ALG_COUNT  4 /**< Number of supported algorithm.*/
...
typedef struct _PUBLICKEY_BLOB {
    ...
    BYTE			*pubKey;		/**< The public key */
    DWORD			dwPubKeyLen;	/**< The public key length.*/
} PUBLICKEY_BLOB;


/** \brief Key container structure.
 *
 *  This a representation of the keys (exchange and signature).
 *  \ingroup    SPInternal
 */
...
typedef struct  _KEY_INFO {
    ...
    DWORD			     fLen;  /**< If mode is OFB or CFB, feedback length in
                                     bits.*/
	BOOL			exportable;	/**< If Key has flag CRYPT_EXPORTABLE.*/
...
typedef struct _HASH_INFO {
	ALG_ID		algid;			/**< Hash algorithm ID */
	DWORD		dwHashLen;		/**< Hash length */
	BOOL		finished;		/**< TRUE if hash has been finished*/
	HANDLE		hHashInformation;/**< HASH speific information*/
} HASH_INFO;

/** \brief Provider information
 *	
 *	Contents al necessary information to perfom CryptoAPI operations
 *
 *	\ingroup SPInternal
 */
typedef struct _PROV_CTX {

	CONTAINER_INFO *pContainer;		/**< using container*/
...
/** \brief  Sign digitaly a hash object.
 *
 *  Typicals steps:
 ...
 */
BOOL reverseBytesString(BYTE *pBytes, DWORD stringLen);
